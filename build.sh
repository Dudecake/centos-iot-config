...
RELEASEVER="$(grep -Po '(?<=releasever: ")[^"]+' ${DIR}/centos-common-ostree.yaml)"
MACHINE="$(uname -m)"
...
if [[ -f "${DIST_PATH}/refs/heads/centos/${RELEASEVER}/${MACHINE}/${DIST_NAME}" ]]; then
  NEW_REPO=1
fi
cd ${DIR}
...
# ostree pull centos-iot:centos/${RELEASEVER}/${MACHINE}/${DIST_NAME}
# ostree admin deploy --os=centos-iot centos-iot:centos/${RELEASEVER}/${MACHINE}/${DIST_NAME}
